  ...
  return (
    <div className="flex flex-wrap items-center p-4 md:p-6 relative overflow-hidden">
      ...
      <div className="flex-1 min-w-[300px] z-10">
        <h2 className="text-xl md:text-3xl text-gray-700 mb-1">Upgrade your skill for better</h2>
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800 mb-6 md:mb-8">FUTURE</h1>

        <div className="relative flex gap-6 md:gap-8 mb-6 md:mb-8">
          {tabs.map((tab) => (
            <div
              key={tab}
              className={`px-2 md:px-4 py-2 cursor-pointer font-medium ${
          ...
          <div
            className="absolute bottom-0 h-1 w-16 md:w-20 bg-purple-600 rounded-md transition-all duration-300"
            style={{ left: `${tabs.indexOf(activeTab) * (window.innerWidth > 768 ? 96 : 72)}px` }}
          ></div>
        </div>
